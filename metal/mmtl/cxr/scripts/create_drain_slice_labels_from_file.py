...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--positive", "-p", type=int, help="Positive (1) or negative (0)", required=True
    )
    parser.add_argument("--split", "-s", help="Split to operate on", required=True)
    parser.add_argument(
        "--only_slice",
        "-os",
        help="if True, only labels from slice; else, labels from slice plus all other classes as negative",
        type=int,
        required=True,
    )
    ...
    split_drains = split_pneumos[
        split_pneumos["Image Index"].isin(drains["Image Index"])
    ]["Image Index"]
    split_no_drains = split_pneumos[
        split_pneumos["Image Index"].isin(no_drains["Image Index"])
    ]["Image Index"]
    ...
    slice_df.to_csv(
        f"{args.out_dir}/{split}_gt_os_{args.only_slice}.tsv", sep="\t", index=False
    )

    logger.info(
        f"{np.sum(slice_df['slice_label'])} pos={args.positive} labels in {split} split"
    )
