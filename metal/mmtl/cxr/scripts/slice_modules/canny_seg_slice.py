...
from keras.models import load_model  # pretrained segnet
...
class CannySegSliceModule(nn.Module):
    def __init__(
        ...
        lng_cutoff=40,
        slope_cutoff=0.5,
        canny_kwargs={"apertureSize": 3, "L2gradient": True},
    ):
    ...
    def get_hough_lines_2(self, x):
        lines_2 = cv2.HoughLinesP(
            x, rho=1, theta=1 * np.pi / 180, threshold=4, minLineLength=10, maxLineGap=4
        )
    ...
    def forward_fun(self, args):
        ...
        diff_img, lungs, edges = self.get_canny_seg_diff_image(
            x, kwargs=self.canny_kwargs
        )
        ...
        morph_clos_1 = cv2.morphologyEx(
            hough_line_image_1, cv2.MORPH_CLOSE, kernel=np.array([2, 10]), iterations=10
        )
        ...
        sorted_components = Counter(labels.flatten()).most_common()[
            1:
        ]  # Ignoring background component
