...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--rcd_dir",
        "-rcd",
        help="Directory containing NIH data from reproduce-chexnet",
        required=True,
    )
    parser.add_argument(
        "--drain_dir",
        "-dd",
        help="Directory containing list of confirmed chest drains",
        required=True,
    )
    parser.add_argument(
        "--out_dir", "-od", help="Output directory for adjusted data files"
    )
    ...
    drain_swap_subset = dfs["test"][
        dfs["test"]["Image Index"].isin(drain_swap_data["Image Index"])
    ]
    ...
    dfs["dev"] = dfs["dev"][
        ~dfs["dev"]["Image Index"].isin(dev_swap_subset["Image Index"])
    ]
    dfs["test"] = dfs["test"][
        ~dfs["test"]["Image Index"].isin(drain_swap_subset["Image Index"])
    ]
    ...
    test_drains = len(
        dfs["test"][dfs["test"]["Image Index"].isin(drains["Image Index"])]
    )
    ...
    # Number in original subset in dev set:
    dev_no_drains = len(
        dfs["dev"][dfs["dev"]["Image Index"].isin(no_drains["Image Index"])]
    )
    test_no_drains = len(
        dfs["test"][dfs["test"]["Image Index"].isin(no_drains["Image Index"])]
    )
    ...
    for split in dfs:
        dfs[split].to_csv(
            os.path.join(args.out_dir, f"{split}.tsv"), sep="\t", index=False
        )
