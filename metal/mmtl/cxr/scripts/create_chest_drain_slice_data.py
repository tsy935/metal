...
if __name__ == "__main__": 
    ...
    drain_train_plus_val, dfs_out["test"] = train_test_split(drain_data_tot, test_size=0.1)
    dfs_out["train"], dfs_out["dev"] = train_test_split(drain_train_plus_val, test_size=0.1)

    for split in ["train", "dev", "test"]:
        st = dfs_out[split]
        logger.info(f"{split} set: {len(st)} Examples, {100*np.sum(st['drain'])/len(st):0.2f} Percent Positive")
