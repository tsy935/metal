...
parser.add_argument(
    "--path_to_images", "-ip", required=True, type=str, help="path to image data"
)
parser.add_argument(
    "--path_to_labels", "-lp", required=True, type=str, help="path to label directory"
)
parser.add_argument("--config", required=True, type=str, help="path to config dict")
...
def train_model():
    ...
    for split in splits:
        ...
        dataloaders[split] = DataLoader(
            dataset, **em_config["train_config"]["data_loader_config"]
        )
    ...
    em_config["train_config"]["checkpoint_config"]["checkpoint_dir"] = os.path.join(
        searcher.log_subdir, "checkpoints"
    )
    ...
    end_model = searcher.search(
        ...
        train_kwargs=em_config["train_config"],
    ...
    end_model.score(
        val_loader, metric=["accuracy", "precision", "recall", "f1", "roc-auc"]
    )
